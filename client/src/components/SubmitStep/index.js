...
const SubmitStep = ({ submission = {}, result, children }) => {
  ...
  for( let n=0; n<submission.genes.length; n++ ) {
     if(!submission.genes[n].hasSummary) {
        ...
        let wufooURL = 'https://flybase.wufoo.com/forms/flybase-gene-snapshot-form';
        ...
        noSumGeneList.push(<li><a href={encodeURI(wufooURL)} target="_blank"><b>{gn.symbol}</b> <i class="fa fa-external-link" /></a></li>)
     }
  }
  ...
  let sumInvite = (
    <div class="panel panel-default">
      <div class="panel-heading">
        <h4>FlyBase Gene Snapshots</h4>
      </div>
      <div class="panel-body">
        Based on the genes associated with your recent publication,
        you've done work on some genes that do not currently have{' '}
        <a href="https://wiki.flybase.org/wiki/Gene_Snapshots">Gene Snapshots</a>.
        Would you like to contribute a Gene Snapshot for any or all of the following genes?
        <ul>{ noSumGeneList }</ul>
      </div>
      <div class="panel-footer text-primary">
        Links will open a new tab/window containing a form hosted at wufoo.com for FlyBase.
      </div>
    </div>
  )
  return (
